<template>
  ...
  <div class="patientNotesOuter timeline">
    <div class="filterBy">
      <a-tag closable v-for="filter, index in filtersName" :key="index" @close="handleClose(filter.name)">{{ filter.name }}</a-tag>
      <a-dropdown-button :trigger="['click']">
        Filter By
        <template #overlay>
          <a-menu class="filterMenu">
            <a-checkbox-group v-model:value="filters" @change="handleChange">
              <a-checkbox class="custom-checkbox" v-for="filter in store.state.patientsTimeLine.patientsTimeLineFilter"
                :key="filter.id" :value="filter.id">{{ filter.name }}</a-checkbox>
            </a-checkbox-group>
          </a-menu>
        </template>
        <template #icon>
    ...
    <div>
      <EmptyTable v-if="store.state.patientsTimeLine.patientsTimeLineList.length == 0" :text="$t('global.timelineTab')" />
      <a-table v-if="store.state.patientsTimeLine?.patientsTimeLineList?.length > 0"
        :dataSource="store.state.patientsTimeLine.patientsTimeLineList" :columns="columns" class="common-table no-link"
        :pagination="false" @change="tableSorting">
        <template #bodyCell="{ column, text ,record }">
          <template v-if="column.dataIndex == 'createdBy'">
            <span>{{ text }} {{record.roleName ? '(' + record?.roleName + ')' : ''}}</span>
    ...
    <a-col :span="24" v-if="store.state.patientsTimeLine.patientsTimeLineList?.length > 0">
      <Pagination v-if="store.state.patientsTimeLine.patientsTimeLineMeta"
        :meta="store.state.patientsTimeLine?.patientsTimeLineMeta" commit="PATIENTS_TIMELINE_LIST"
        :endpoint="`patientData/list/patient/${route?.params?.udid}/timeLine`" :params="'&type=' + typesWithString"
...
const handleClose = removedTag => {
  ...
  filtersName.value = filtersName.value.filter(tag => tag.name !== removedTag);
   const tags = filterIdModification()
 // filters.value = tags;
  typesWithString.value = tags?.length > 0 ? tags?.toString() : ''
  
  store.dispatch("patientsTimeLineList", { id: route?.params?.udid, filter: '?type=' + typesWithString.value +store.getters.orderTable.value?.data})
};
const handleChange = () => {
  //store.state.patientsTimeLine.patientsTimeLineMeta = null
  typesWithString.value= filters.value?.length > 0 ? filters.value?.toString() : ''
  filterModification()
  store.dispatch("patientsTimeLineList", { id: route?.params?.udid, filter: '?type=' + typesWithString.value +store.getters.orderTable.value?.data })
};
...
onMounted(async () => {
  ...
  store.dispatch("patientsTimeLineList", { id: route?.params?.udid, filter: '?type=' + '' })
